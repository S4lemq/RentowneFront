  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    return next.handle(req).pipe(
      catchError((error: HttpErrorResponse) => {
        if (error.error && ((error.error.message === 'WRONG_GOOGLE_AUTH_CODE') || (error.error.message === 'BAD_CREDENTIALS') || (error.error.message === 'USER_ALREADY_EXISTS') || (error.error.message === 'PROVIDER_TYPE_ALREADY_EXISTS') || (error.error.message === 'BAD_OLD_PASSWORD'))) {
