...
@Component({
  selector: 'app-lease-agreement-add',
  templateUrl: './lease-agreement-add.component.html',
  styleUrls: ['./lease-agreement-add.component.scss']
})
export class LeaseAgreementAddComponent implements OnInit, OnDestroy {

  private killer$ = new Subject<void>();
  leaseAgreementForm!: FormGroup;
  leasesNumbers: number[] = [];
  ...
  ngOnInit(): void {
    ...
    this.leasesNumbers = [...Array.from({ length: 28 }, (_, index) => index)];
  }
  ...
  submit() {
    if (this.leaseAgreementForm.valid) {
      ...
      .subscribe(leaseAgreement => {
        const translatedText = this.translateService.instant("snackbar.leaseAgreementAdded");
        this.router.navigate(["lease-agreements/edit/", leaseAgreement.id])
